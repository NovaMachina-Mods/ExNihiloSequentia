...
public class Mekanism implements IDefaultRegistry {
    ...
    public void registerSieve(SieveRegistry registry) {
        ...
        if (!EnumOre.COPPER.isEnabled()) {
            registry.addMultiMeshDrop(Blocks.GRAVEL, EnumOre.TIN.getPieceItem()
                .get(), null, 0.05F, 0.075F, null, false);
            EnumOre.COPPER.setEnabled();
        }

        if (!EnumOre.OSMIUM.isEnabled()) {
            registry
                .addDrop(Blocks.GRAVEL, EnumOre.OSMIUM.getPieceItem().get(), 0.05F, EnumMesh.IRON, false);
            registry.addDrop(Blocks.GRAVEL, EnumOre.OSMIUM.getPieceItem()
