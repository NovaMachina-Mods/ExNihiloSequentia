...
public class PebbleItem extends SnowballItem {
    ...
    @Override
    @Nonnull
    public InteractionResultHolder<ItemStack> use(@Nonnull final Level world, @Nonnull final Player player,
        ...
        world.playSound(null, player.getX(), player.getY(), player.getZ(), ExNihiloSounds.PEBBLE_THROW,
                SoundSource.NEUTRAL, 0.5F, 0.4F / (new Random().nextFloat() * 0.4F + 0.8F));
        if (!world.isClientSide) {
            @Nonnull final Snowball snowballentity = new Snowball(world, player){
                @Override
            ...
            snowballentity.shootFromRotation(player, player.xRotO, player.yRotO, 0.0F, 1.5F, 1.0F);
