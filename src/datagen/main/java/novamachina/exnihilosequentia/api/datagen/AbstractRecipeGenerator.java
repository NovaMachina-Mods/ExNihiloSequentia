...
public abstract class AbstractRecipeGenerator extends RecipeProvider {
  ...
  protected AbstractRecipeGenerator(@Nonnull final DataGenerator generator,
  ...
  protected void createOreRecipes(Ore ore, ResourceLocation registryId,
      Consumer<FinishedRecipe> consumer) {
    createOre(ore, consumer);
    createSmeltingRecipe(consumer, ore.getRawOreItem(), ore.getIngotItem(), 0.7F, 200, 0.7F, 100,
        CHUNK_CONDITION, registryId);
  }

  protected void createOre(@Nonnull final Ore ore,
  ...
  protected void createOre(@Nonnull final Ore ore, @Nonnull Item rawOre,
      @Nonnull final Consumer<FinishedRecipe> consumer) {
  ...
  protected void createSmelting(@Nonnull final Ore ore,
    ...
    SimpleCookingRecipeBuilder
        .smelting(Ingredient.of(chunk), ingot, 0.7F, 200)
        .unlockedBy(CHUNK_CONDITION,
            InventoryChangeTrigger.TriggerInstance.hasItems(ore.getRawOreItem()))
        .save(consumer, new ResourceLocation(modId, prependRecipePrefix(ore.getIngotName())));
    SimpleCookingRecipeBuilder
        .blasting(Ingredient.of(chunk), ingot, 0.7F, 100)
        .unlockedBy(CHUNK_CONDITION,
            InventoryChangeTrigger.TriggerInstance.hasItems(ore.getRawOreItem()))
        .save(consumer,
  ...
  protected void createCrookRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
    ...
    CrookRecipeBuilder.builder().input(itemInput).addDrop(itemDrop, chance)
  ...
  protected void createFiredCrucibleRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
    ...
    CrucibleRecipeBuilder.builder().input(Ingredient.of(block)).amount(amount)
  ...
  protected void createFiredCrucibleRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
      @Nonnull final TagKey<Item> item, final int amount,
      @Nonnull final String id) {
    CrucibleRecipeBuilder.builder().input(Ingredient.of(item)).amount(amount)
        .fluidResult(Fluids.LAVA)
        .crucibleType(CrucibleTypeEnum.FIRED).build(consumer, crucibleLoc(id));
  }
  ...
  protected void createFluidItemRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
    ...
    FluidItemRecipeBuilder.builder().fluidInBarrel(fluidInput).input(itemInput).result(blockOutput)
  ...
  protected void createFluidOnTopRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
      ...
      @Nonnull final Block blockOutput, @Nonnull final String id) {
    FluidOnTopRecipeBuilder.builder().fluidInTank(fluidInTank).fluidOnTop(fluidOnTop)
        .result(blockOutput)
  ...
  protected void createHammerRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
      ...
      @Nonnull final String id) {
    HammerRecipeBuilder.builder().input(blockInput).addDrop(blockOutput)
        .build(consumer, hammerLoc(id));
  }

  protected void createHeatRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
      @Nonnull final Block block,
  ...
  protected void createHeatRecipes(@Nonnull final Consumer<FinishedRecipe> consumer,
      ...
      @Nonnull final String id) {
    HeatRecipeBuilder.builder().input(block).amount(amount).properties(properties)
        .build(consumer, heatLoc(id));
  }

  @SuppressWarnings("SameParameterValue")
  protected void createSmeltingRecipe(@Nonnull final Consumer<FinishedRecipe> consumer,
  ...
  @SuppressWarnings("SameParameterValue")
  protected void createCookingRecipe(@Nonnull final Consumer<FinishedRecipe> consumer,
      @Nonnull final Item input,
      ...
      @Nonnull final ResourceLocation rl) {
    SimpleCookingRecipeBuilder.cooking(Ingredient.of(input), output, xpCampfire, durationCampfire,
            RecipeSerializer.CAMPFIRE_COOKING_RECIPE)
        .unlockedBy(condition, InventoryChangeTrigger.TriggerInstance.hasItems(input))
        .save(consumer, createSaveLocation(new ResourceLocation(rl + "_from_campfire")));
    SimpleCookingRecipeBuilder.cooking(Ingredient.of(input), output, xpSmoker, durationSmoker,
